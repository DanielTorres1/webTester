...
def usage():
    ...
    parser.add_argument("infile", nargs='?', help="File with domain or URL list")
    parser.add_argument("-u", metavar="url", help="Single URL to check")
...
def main():
    try:
        if args.u:
            ...
            raise ValueError("No URL or infile provided")
    ...
    try: 
        pool.map_async(check, urls).get(timeout=10)  # Assuming timeoutValue is defined as 10 or another integer
...
def alert(url, msg): 
    print("{} -Vulnerable: Alert: {}".format(url, msg))

def invalid(url, msg): 
    print("{} - Invalid: {}".format(url, msg))

def warning(url, msg): 
    print("{} -Vulnerable: Warning: {}".format(url, msg))

def notvuln(url, msg): 
    print("{} - OK: {}".format(url, msg))
