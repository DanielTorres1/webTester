...
func HeaderInject() {
	...
	for _, header := range headers {
		for _, i := range inject {
			...
			tolerance := 0.003 * float64(bContentLength)
			if abs(bContentLength-contentLength) >= int64(tolerance) {

				g.Println("[Vul] " + "[" + urlt + "]" + " " + "[" + header + ": " + i + "]" + " " + "[Code: " + strconv.Itoa(int(resp.StatusCode)) + "] " + "[Size: " + strconv.Itoa(int(contentLength)) + "]")
			} else {
				r.Println("[OK] " + "[" + urlt + "]" + " " + "[" + header + ": " + i + "]" + " " + "[Code: " + strconv.Itoa(int(resp.StatusCode)) + "] " + "[Size: " + strconv.Itoa(int(contentLength)) + "]")
			}
			defer resp.Body.Close()
		}
	}
